...
def get_parser_service(llm_provider: LLMProvider, use_vision: bool = True):
    ...
    if llm_provider == LLMProvider.openai:
        return (
            BankStatementParserFactory.create_openai_vision_parser()
            if use_vision
            else BankStatementParserFactory.create_openai_text_parser()
        )
...
async def parse_bank_statement(
    ...
    use_vision: bool = Query(
        default=True, description="Use vision model (True) or text extraction (False)"
    ),
    llm_provider: LLMProvider = Query(
        default=LLMProvider.openai, description="Choose which LLM provider to use"
    ),
) -> JSONResponse:
    """
    ...
    try:
        logger.info(
            f"Processing file: {file.filename}, use_vision: {
                use_vision
        ...
        if result.success:
            logger.info(
                f"Successfully parsed statement with {llm_provider} in {
                    result.processing_time:.2f}s"
            )
            return JSONResponse(status_code=200, content=result.dict())
        else:
            logger.error(
                f"Failed to parse statement with {llm_provider}: {result.error}"
            )
            return JSONResponse(status_code=422, content=result.dict())

    except Exception as e:
        logger.error(f"Unexpected error with {llm_provider}: {str(e)}")
        error_response = ParsedResponse(
            success=False, error=f"Unexpected error with {llm_provider}: {str(e)}"
        )
