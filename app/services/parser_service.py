...
class BankStatementParserService:
    ...
    def __init__(
        ...
        self.pdf_processor = (
            pdf_processor or PDFProcessorFactory.create_image_processor()
        )
        self.llm_service = llm_service or LLMServiceFactory.create_openai_service()
        self.file_validator = (
            file_validator or FileValidationFactory.create_default_validation_service()
        )

    async def parse_statement(
        self, file: UploadFile, use_vision: bool = True
    ) -> ParsedResponse:
        """
        ...
        try:
            ...
            return ParsedResponse(
                success=True, data=bank_statement, processing_time=processing_time
            )
